...
class NeuralNetwork(ABC):
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        for forward_layer in self.layers:
            X = forward_layer.forward(X)
        return X
    ...
    def backward(self, target: np.ndarray, out: np.ndarray) -> float:
        ...
        backprop = self.loss.backward(target,out)
        for backward_layer in reversed(self.layers):
            backprop = backward_layer.backward(backprop)
        return model_loss
