...
class FullyConnected(Layer):
    ...
    def backward(self, dLdY: np.ndarray) -> np.ndarray:
        ...
        self.gradients["b"] = np.sum(dLdZ,axis=0).reshape(self.gradients["b"].shape)
        self.gradients["W"] = self.cache["X"].T@dLdZ
        ### END YOUR CODE ###
...
class Conv2D(Layer):
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        X,_ = pad2d(X, self.pad, self.kernel_shape, self.stride)

        rows = math.floor((((2*self.pad[0]+in_rows)-kernel_height)/self.stride))
        cols = math.floor((((2*self.pad[1]+in_cols)-kernel_width)/self.stride))
        Z = np.zeros((n_examples,rows+1,cols+1,out_channels))

        for i in range(rows+1):
            row_idx = i*self.stride
            for j in range(cols+1):
                col_idx = j*self.stride
                for k in range(out_channels):
                    Z[:,i,j,k] = np.sum(W[:,:,:,k]*X[:,row_idx:(row_idx+kernel_height),col_idx:(col_idx+kernel_width)],axis=(1,2,3))
                    Z[:,i,j,k] += b[:,k]
    ...
    def backward(self, dLdY: np.ndarray) -> np.ndarray:
        ...
        for i in range(rows+1):
            ...
            for j in range(cols+1):
                col_idx = j *self.stride
                for k in range(out_channels):
                    dLdW[:,:,:,k] += np.sum(X[:,row_idx:(row_idx+kernel_height),col_idx:(col_idx+kernel_width),:]*dLdZ[:,i:i+1,j:j+1,np.newaxis,k],axis=0)
                    dLdX[:,row_idx:(row_idx+kernel_height),col_idx:(col_idx+kernel_width)] += W[np.newaxis,:,:,:,k]*dLdZ[:,i:i+1,j:j+1,np.newaxis,k]

        self.gradients["b"] = dLdZ.sum(axis=(0,1,2)).ravel()
        self.gradients["W"] = dLdW

        ### END YOUR CODE ###

        return dLdX[:,self.pad[0]:self.pad[0]+in_rows,self.pad[1]:self.pad[1]+in_cols]
...
class Pool2D(Layer):
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        rows = math.floor((((2*self.pad[0]+in_rows)-kernel_height)/self.stride))
        cols = math.floor((((2*self.pad[1]+in_cols)-kernel_width)/self.stride))
        Z = np.zeros((n_examples,rows+1,cols+1,in_channels))

        for i in range(rows+1):
            row_idx = i*self.stride
            for j in range(cols+1):
                col_idx = j*self.stride
                Z[:,i,j,:] = self.pool_fn(X[:,row_idx:(row_idx+kernel_height),col_idx:(col_idx+kernel_width)],axis=(1,2))
        self.cache["Z"] = Z
        return Z
