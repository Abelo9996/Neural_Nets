...
class SoftMax(Activation):
    ...
    def forward(self, Z: np.ndarray) -> np.ndarray:
        ...
        f(z) as described above applied elementwise to `Z`
        """
        m = np.amax(Z, axis=1).reshape(Z.shape[0],1)
        return np.exp(Z-m)/np.sum(np.exp(Z-m),axis=1).reshape(Z.shape[0],1)
    ...
    def backward(self, Z: np.ndarray, dY: np.ndarray) -> np.ndarray:
        ...
        derivative of loss w.r.t. input of this layer
        """
        ret_arr = self.forward(Z)
        idx = ret_arr.shape[0]*ret_arr.shape[1]
        for i,c in enumerate(ret_arr):
            c = c.ravel().reshape(Z.shape[1], 1)
            a = np.zeros((Z.shape[1], Z.shape[1]))
            np.fill_diagonal(a, c)
            ret_arr = np.append(ret_arr,(a - c@c.T)@dY[i])
        return ret_arr[idx:].reshape(Z.shape)
